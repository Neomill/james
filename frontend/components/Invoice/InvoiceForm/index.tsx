import { Button } from "@/components/Button";
import Select from "@/components/Select";
import {
  useGetInvoiceByIdQuery,
  useUpdateInvoiceMutation,
} from "@/redux/services/invoicesAPI";

import { yupResolver } from "@hookform/resolvers/yup";
import React, { useEffect } from "react";
import { FormProvider, useForm } from "react-hook-form";
import { toast } from "react-toastify";
import * as yup from "yup";

const InvoiceSchema = yup
  .object({
    status: yup.string().required(),
  })
  .required();

interface Props {
  onClose: () => void;
  id?: string;
}

export const InvoiceForm: React.VFC<Props> = ({ onClose, id }) => {
  const methods = useForm({
    resolver: yupResolver(InvoiceSchema),
  });
  const [update, result] = useUpdateInvoiceMutation();

  if (id) {
    const { data: invoice, isLoading: isUpdating } = useGetInvoiceByIdQuery(
      id,
      {
        pollingInterval: 3000,
        refetchOnMountOrArgChange: true,
        skip: false,
      }
    );
    useEffect(() => {
      if (invoice) {
        const { setValue } = methods;
        setValue("status", invoice.status);
        setValue("payment_status", invoice.payment_status);
      }
      return () => {};
    }, [id, invoice]);
  }

  const onSubmit = async (data: any) => {
    try {
      if (id)
        toast.promise(update({ id, ...data }).unwrap(), {
          success: "Invoice updated successfully!",
          error: "Error updating invoice!",
          pending: "Updating invoice...",
        });
      onClose();
    } catch (error) {
      console.error(error.message);
    }
  };

  return (
    <FormProvider {...methods}>
      <form onSubmit={methods.handleSubmit(onSubmit)}>
        <div className={" border-0 flex flex-col gap-9"}>
          <div
            className={["justify-between flex flex-col md:flex-row gap-3"].join(
              " "
            )}
          >
            <div className="w-96 flex flex-col gap-2 ">
              <Select
                options={[
                  {
                    name: "In Progress",
                    id: "IN_PROGRESS",
                  },
                  {
                    name: "Ready",
                    id: "READY",
                  },
                  {
                    name: "Void",
                    id: "VOID",
                  },
                ]}
                name="status"
                placeholder="Select Status"
                label="Status"
              />
            </div>
          </div>
          <div className="flex flex-col  justify-end md:flex-row gap-3">
            <Button
              type="submit"
              width="wide"
              size="medium"
              label={id ? "Update" : "Submit"}
            />
          </div>
        </div>
      </form>
    </FormProvider>
  );
};
