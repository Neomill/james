...
const Categories = () => {
  ...
  const { data, error, isLoading } = useSearchCategoryQuery({
    page,
    query,
    ...filters,
    ...sortBy,
      ...
      },
      {
        ...
        Cell: (props: any) => (
          <div className=" flex flex-row gap-4 items-center">
            <ActionButton
              data-modal="view-category-modal"
              onClick={(e) => onModalOpen(e, props.row.original.id)}
              action="view"
            />
            {checkPermissions(["update-category"], user.roles) && (
              <ActionButton
            ...
            {checkPermissions(["delete-category"], user.roles) && (
              <ActionButton
  ...
  if (!checkPermissions(["read-category"], user.roles)) {
    return <Forbidden />;
  }
    ...
    <>
      <FormProvider {...methods}>
        <form onSubmit={methods.handleSubmit(onSubmitSearch)}>
          ...
          >
            {checkPermissions(["create-category"], user.roles) && (
              <>
                <Button
                  data-modal="new-category-modal"
                  onClick={onModalOpen}
                  icon={<BsPlusCircle />}
                  size="medium"
                  label="Add Category"
...
Categories.getLayout = function getLayout(page: ReactElement) {
  return (
    <Layout icon={<BsListUl />} title="Categories">
      {page}
    </Layout>
  );
};
